      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 p-5 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col justify-between">
          ...
          <div className="pt-4 flex items-start gap-2 text-xs text-slate-500 leading-relaxed font-serif">
            ...
            <p>
              {processMathText(`Even though the company continues paying dividends forever, their Present Value shrinking to zero proves why the total sum is <span className="font-bold text-slate-800">finite</span>. The sum of these heights is exactly equal to the calculated stock price of <span className="font-bold text-indigo-600">${formatCurrency(theoreticalPrice)}</span>.`)}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans text-slate-600">
          <div className="space-y-1.5">
            ...
            <p className="leading-relaxed">
              {processMathText('When the required return $r$ approaches the growth rate $g$ from above, the denominator $(r - g)$ approaches zero. Mathematically, dividing by a number close to zero causes the stock price to <span className="font-bold text-slate-800">skyrocket toward infinity</span>. This is why the stock market goes into wild swings when growth expectations drop by even 0.5%—the math multiplies tiny changes dramatically!')}
            </p>
          </div>
          <div className="space-y-1.5">
