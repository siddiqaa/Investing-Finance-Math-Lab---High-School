...
When you add all these ongoing operational expenses to your monthly loan payment, you arrive at the Total Cost of Ownership (TCO). In this lesson, we will use multi-variable algebra to analyze how different car choices—such as a gas-powered car with high MPG versus an electric vehicle with low maintenance—behave over time. By mastering this math, you will avoid the "sticker price trap" and learn to make smart, mathematically sound car buying decisions.`,
  fullText: [
    ...
    '<span className="text-amber-700 font-bold">Common Mistake:</span> Students often think stretching a loan from 60 months to 84 months (7 years) saves money because the monthly payment drops. In reality, stretching the loan increases the total interest paid drastically and leaves you "underwater" (owing more on the loan than the car is worth on the market).',

    '2. <span className="text-indigo-600 font-bold">Total Cost of Ownership (TCO) Multi-Variable Formula</span>\nThe monthly loan payment is only one component of vehicle ownership. The complete monthly cost function $C_{\\text{monthly}}$ is:\n$C_{\\text{monthly}} = PMT + C_{\\text{insurance}} + C_{\\text{fuel}} + C_{\\text{maintenance}}$',

    'Let me break down how operational costs are modeled mathematically:',
    '• <span className="font-bold text-slate-800">Fuel Cost (Gasoline ICE):</span> $C_{\\text{fuel}} = \\left(\\frac{\\text{Monthly Miles}}{MPG}\\right) \\times \\text{Gas Price (\\$/gal)}$\n• <span className="font-bold text-slate-800">Energy Cost (Electric EV):</span> $C_{\\text{electric}} = \\left(\\frac{\\text{Monthly Miles}}{100}\\right) \\times \\text{kWh per 100 mi} \\times \\text{Electricity Price (\\$/kWh)}$\n• <span className="font-bold text-slate-800">Insurance ($C_{\\text{insurance}}$):</span> High school and teen drivers average \\$200–\\$350/month due to risk tiering. EV insurance can sometimes be 10-15% higher due to costly battery replacement parts.\n• <span className="font-bold text-slate-800">Maintenance & Reliability ($C_{\\text{maintenance}}$):</span> ICE cars require regular oil changes, spark plugs, timing belts, and brake servicing (\\$60–\\$120/mo average). EVs have no engine oil or spark plugs and use regenerative braking to preserve pads, leading to lower routine service costs (\\$30–\\$60/mo average), though battery degradation and tire wear must be monitored.',

    '3. <span className="text-indigo-600 font-bold">Head-to-Head Comparison: Gas (ICE) vs. Electric (EV)</span>\nLet\'s compare two real-world choices for a student driving 1,000 miles per month over a 5-year (60-month) ownership window:',
    ...
    '<span className="font-bold text-slate-800">Key Insight:</span> Even though the EV had a <span className="font-bold text-slate-800">\\$4,000 higher sticker price</span> and a higher loan payment (\\$386.66 vs \\$309.33), its lower fuel and maintenance costs saved <span className="font-bold text-indigo-600">\\$91.33/month</span> in operations! Over 5 years, the EV actually cost <span className="font-bold text-indigo-600">\\$2,040 LESS total</span> than the cheaper gas car. Math allows you to see beyond the sticker price.',

    '<span className="font-bold text-slate-800">Real-World Experiment:</span> What happens if gas prices drop to \\$2.80/gallon or if you drive only 500 miles/month? The gas car becomes cheaper! TCO isn\'t fixed; it is a dynamic function of your driving habits and local energy rates.'
  ],
  quizzes: [
    {
      id: 'amortization_q1',
